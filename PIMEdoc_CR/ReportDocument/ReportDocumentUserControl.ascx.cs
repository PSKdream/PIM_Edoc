...
{
    public partial class ReportDocumentUserControl : UserControl
    {
        ...
        protected void InitialData()
        {
            ...
            rdb_FilterByEmpDept.SelectedIndex = 0;
            rdb_FilterByEmpDept_SelectedIndexChanged(new object(), new EventArgs());
        ...
        protected void GenerateData(string sType)
        {
            try
            {
                ...
                {
                    oDocType = ddl_DocType.SelectedValue;
                    listDocument = listDocument.Where(x => x.DocTypeCode == oDocType).ToList();
                }
                #endregion
                string oData = "";
                switch (ddl_FilterByTimePeriod.SelectedValue)
                {
                    ...
                    case "Daily":
                        ...
                        listDocument = listDocument
                            .Where(x => ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Date >= dailyStartDate.Date
                                     && ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Date <= dailyEndDate.Date)
                                     .ToList();
                    ...
                    case "Weekly":
                        ...
                        listDocument = listDocument
                            .Where(x => ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Date >= weeklyStartDate.Date
                                     && ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Date <= weeklyEndDate.Date)
                            .ToList();
                    ...
                    case "Month":
                        listDocument = listDocument
                            .Where(x => ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Year == Convert.ToInt32(ddl_MonthlyYear.SelectedValue))
                            .ToList();
                    ...
                    case "Monthly":
                        listDocument = listDocument
                            .Where(x => ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Month >= Convert.ToInt32(ddl_OnMonth.SelectedValue)
                                    && ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Year <= Convert.ToInt32(ddl_OnMonthYear.SelectedValue))
                            .ToList();
                    ...
                    case "Year":
                        listDocument = listDocument
                            .Where(x => ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Year >= Convert.ToInt32(ddl_FromYear.SelectedValue)
                                    && ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Year <= Convert.ToInt32(ddl_ToYear.SelectedValue))
                            .ToList();
        ...
        protected string ConvertListToSeries(List<PIMEdoc_CR.TRNDocument> sListDocument, int sStart, int sEnd, string sDocTypeCode, bool isYear, bool isMonth, bool isInMonth, DateTime? sStartDate)
        {
            ...
            for (int i = sStart; i <= sEnd; i++)
            {
                foreach (var serie in oSeries)
                {
                    ...
                    if (isYear)
                    {
                        tempList = tempList.Where(x => ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Year == i).ToList();
                    }
                    else if (isMonth)
                    {
                        tempList = tempList.Where(x => ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Month == i).ToList();
                    }
                    else if (isInMonth)
                    {
                        tempList = tempList.Where(x => ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Day == i).ToList();
                    }
                    else
                    {
                        if (sStartDate != null)
                        {
                            tempList = tempList.Where(x => ((DateTime)(x.ApproveDate ?? DateTime.MinValue)).Date == ((DateTime)sStartDate).AddDays(i)).ToList();
                        }
                    }
